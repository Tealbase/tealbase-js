...
export default function Account({
  ...
  async function uploadAvatar(event: ChangeEvent<HTMLInputElement>) {
    try {
      ...
      let { error: uploadError } = await tealbase.storage
      ...
      let { error: updateError } = await tealbase.from('profiles').upsert({
        id: user!.id,
        avatar_url: filePath,
      })
  ...
  async function getProfile() {
    try {
      setLoading(true)
      const user = tealbase.auth.user()

      let { data, error } = await tealbase
        .from('profiles')
