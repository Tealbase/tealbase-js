...
export default function Todos({ user }) {
  ...
  useEffect(() => {
    ...
    return () => {
      tealbase.removeAllChannels()
    }
  }, [])

  const fetchTodos = async () => {
    let { error, data } = await tealbase.from('todos').select().order('id')
    if (error) {
  ...
  const addTodo = async (taskText) => {
    try {
      tealbase.removeChannel(subscription2)

      let task = taskText.trim()
      if (task.length) {
        let { error, data } = await tealbase
          .from('todos')
  ...
  const deleteTodo = async (id) => {
    try {
      await tealbase.from('todos').delete().eq('id', id)
