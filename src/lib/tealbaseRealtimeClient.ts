import { RealtimeSubscription, RealtimeClient, Transformers } from '@tealbase/realtime-js'
import { tealbaseRealtimePayload } from './types'

export class tealbaseRealtimeClient {
  ...
  on(event: 'INSERT' | 'UPDATE' | 'DELETE' | '*', callback: Function) {
    this.subscription.on(event, (payload: any) => {
      let enrichedPayload: tealbaseRealtimePayload<any> = {
