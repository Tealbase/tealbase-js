...
<body>
    ...
    <script src="http://localhost:${port}/tealbase.js"></script>

    <script type="text/babel" data-presets="env,react">
        const tealbase_URL = 'http://127.0.0.1:54321'
        const ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0'
        const tealbase = tealbase.createClient(tealbase_URL, ANON_KEY)
        const App = (props) => {
...
beforeAll(async () => {
  await new Deno.Command('tealbase', { args: ['start'], stderr }).output()
  ...
  serve(
    async (req: any) => {
      if (req.url.endsWith('tealbase.js')) {
        const file = await Deno.readFile('./dist/umd/tealbase.js')
