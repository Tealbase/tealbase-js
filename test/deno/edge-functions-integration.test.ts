...
// 1. Deploy the edge functions to a tealbase project
// 2. Set the tealbase_URL and tealbase_ANON_KEY environment variables
...
Deno.test(
  ...
  async (t) => {
    ...
    const tealbase_URL = Deno.env.get('tealbase_URL') || 'http://127.0.0.1:54321'
    const ANON_KEY =
    ...
    try {
      ...
      await t.step('echo function - should echo request body', async () => {
        ...
        const { data, error } = await tealbase.functions.invoke('echo', {
          body: testData,
        })
      ...
      await t.step('status function - should return system status', async () => {
        const { data, error } = await tealbase.functions.invoke('status', {
          body: {},
        })
      ...
      await t.step('should handle non-existent function', async () => {
        const { data, error } = await tealbase.functions.invoke('non-existent-function', {
          body: {},
        })
      ...
      await t.step('should handle concurrent function calls', async () => {
        const promises = Array.from({ length: 5 }, (_, i) =>
          tealbase.functions.invoke('hello', {
